  ...
  <div class="flex h-screen antialiased text-gray-800">
    <div class="flex flex-row w-full overflow-x-hidden">
      <div class="flex flex-col py-8 pl-6 pr-2 w-70 bg-white flex-shrink-0">
        ...
        <!-- О себе -->
        <div
          :class="[userBackground ? 'bg-red-200' : 'bg-indigo-200']"
          class="
            flex flex-col
            border
            items-center
        ...
        >
          <div
            :class="{
              'border-yellow-300 border-2 text-yellow-200':
                !userBackground && userCaptain,
              'border-yellow-500 border-2 text-yellow-500':
                userBackground && userCaptain,
              'font-bold border-2 border-indigo-300 text-indigo-400':
                !userBackground && !userCaptain,
              'font-bold border-2 border-red-300 text-red-400':
                userBackground && !userCaptain,
            }"
            class="
              h-10
        ...
        <div class="flex flex-col mt-8">
          <div class="flex flex-row items-center justify-between text-base">
            ...
            <span
              ...
              "
              >{{
                teams.red.players.length + (this.user.team === "Red" ? 1 : 0)
              }}</span
            >
          </div>
          <div class="flex flex-col space-y-1 mt-4 -mx-2 h-48 overflow-y-auto">
            <div v-for="(player, idx) in teams.red.players" :key="idx">
              ...
              >
                <div
                  :class="[
                    teams.red.captain === player
                      ? 'border-2 border-yellow-400 shadow-inner'
                      : '',
                  ]"
                  class="
                    flex
          ...
          <div
            class="flex flex-row items-center justify-between text-base mt-6"
          >
            ...
            <span
              ...
              "
              >{{
                teams.blue.players.length + (this.user.team === "Blue" ? 1 : 0)
              }}</span
            >
          </div>
          <div class="flex flex-col space-y-1 mt-4 -mx-2 h-50 overflow-y-auto">
            <div v-for="(player, idx) in teams.blue.players" :key="'A' + idx">
              ...
              >
                <div
                  :class="[
                    teams.blue.captain === player
                      ? 'border-2 border-indigo-400 shadow-inner'
                      : '',
                  ]"
                  class="
                    flex
